...
int main(int argc, char * argv[])
{
  ...
  //read_json(argc<=1?"../../shared/data/triangle.json":argv[1],camera,objects);
  read_json(argc <= 1 ? "../../shared/data/inside-a-sphere.json" : argv[1], camera, objects);
  //read_json(argc <= 1 ? "../../shared/data/bunny.json" : argv[1], camera, objects);
	//read_json(argc <= 1 ? "../../shared/data/sphere-packing.json" : argv[1], camera, objects);
	std::cout << "obj total #: " << objects.size() << std::endl;
  int width = 640;
  int height = 360;
  ...
  std::cout << "d: " << camera.d << " h: " << camera.height << " w: " << camera.width << std::endl;
  // For each pixel (i,j)
